...
description of _seqArchR_'s algorithm.
...
# Installation
_seqArchR_ is currently made available via GitHub, thus, you can use the 
following procedure for installing _seqArchR_.
...
In order to demonstrate the efficacy of _seqArchR_, we use _seqArchR_ to cluster DNA 
...
In the version `r packageVersion("seqArchR")`, _seqArchR_ naively returns a result 
...
clusters from the last iteration of seqArchR. This element stores the clustering 
...
[https://snikumbh.github.io/seqArchR/reference/viz_bas_vec_heatmap_seqlogo.html]
...
_seqArchR_ can detect _de novo_ sequence features and simultaneously identify the 
complex interactions of different features together with their positional 
specificities.

Note that the sequence architectures identified by _seqArchR_ have no limitations 
